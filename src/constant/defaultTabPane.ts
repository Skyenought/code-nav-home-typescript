...
import {ICover} from "types/index";
/**
 *  这是主题用于切换的数据源
 *
 * */
...
const sceneryList = Array.from(Array(12), (_v, k) => {
  let num = k + 1;
  return {
    name: `风景 ${num}`,
...
const starList = Array.from(Array(12), (_v, k) => {
  let num = k + 1;
  return {
    name: `星空 ${num}`,
...
const poolList = Array.from(Array(12), (_v, k) => {
  let num = k + 1;
  return {
    name: `淡雅 ${num}`,
...
const comicList = Array.from(Array(12), (_v, k) => {
  let num = k + 1;
  return {
    name: `动漫 ${num}`,
